...
明示的なプログラムの代わりに多数の中間層を持つニューラルネットワーク（NN）を用いて、
入力値から目的とする出力層に変換する数学モデルを構築することである。
...
スライドNo17の図とサンプルコード（3層・複数ユニット）が一致していないように思われます。
* スライドは中間層1つのNN
* サンプルコード（3層・複数ユニット）は中間層2つのNN
...
・二乗する理由：単純な差であれば、各ラベルの誤差で正負両方の値が発生し、全体の誤差を正しく表すのに都合が悪いため、
二乗してそれぞれのラベルでの誤差を正の値になるようにするため
